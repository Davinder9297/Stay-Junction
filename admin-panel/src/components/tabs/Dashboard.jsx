...
import HostelBookingCard from '../dashboard/Hostelbookingcard';
  ...
  return (
    <div>
      <h2 className='text-[20px] text-center font-text-font font-medium py-4'>
        Welcome to Stay Junction — Dashboard
      </h2>
      ...
      {error ? (
        ...
        <div className='flex flex-row flex-wrap items-center justify-between gap-2'>
          ...
          <HostelCard
            loading={loading}
            data={response?.data?.hostel_info}
          />
          ...
          <HostelBookingCard
            loading={loading}
            data={response?.data?.hostel_booking_info}
          />
        </div>
      )}
    </div>
