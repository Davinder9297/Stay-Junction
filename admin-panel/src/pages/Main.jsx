...
import Hostel from '../components/tabs/Hostel';
import HostelOrders from '../components/tabs/Hostelorders';
...
function Main() {
  window.document.title = 'Stay Junction — Main';
  ...
  const handleTabChange = (key) => {
    switch (key) {
      ...
      case '6': {
        navigate('/main/hostel-booking-orders');
        break;
      }
      case '7': {
  ...
  useEffect(() => {
    switch (selectedKeys) {
      case '1': {
        window.document.title = 'Stay Junction — Dashboard';
        break;
      }
      case '2': {
        window.document.title = 'Stay Junction — Users';
        break;
      }
      case '3': {
        window.document.title = 'Stay Junction — Hotel Rooms';
        break;
      }
      case '4': {
        window.document.title = 'Stay Junction — Hostels';
        break;
      }
      case '5': {
        window.document.title = 'Stay Junction — Booking Orders';
        break;
      }
      case '6': {
        window.document.title = 'Stay Junction — Hostel Booking Orders';
        break;
      }
      case '7': {
        window.document.title = 'Stay Junction — Profile';
        break;
      }
      case '8': {
        window.document.title = 'Stay Junction — Logout';
        break;
      }
      default: {
        window.document.title = 'Stay Junction — Dashboard';
    ...
    <Layout className='w-full h-screen'>
        ...
        <Menu
          ...
          items={[
            ...
            },
            {
              ...
              label: 'Room Booking Orders'
            },
            {
              key: '6',
              icon: <FileProtectOutlined />,
              label: 'Hostel Booking Orders'
            },
            {
              key: '7',
              icon: <UserOutlined />,
      ...
      <Layout>
        ...
        <Content className='bg-bg-white overflow-y-scroll m-2 p-2'>
          ...
          {selectedKeys === '4' && (<Hostel />)}
          {selectedKeys === '5' && (<Orders />)}
          {selectedKeys === '6' && (<HostelOrders />)}
          {selectedKeys === '7' && (<MyProfile />)}
        </Content>
