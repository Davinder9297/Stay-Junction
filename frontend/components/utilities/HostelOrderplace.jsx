...
function HostelOrderPlaceModal({ bookingModal, setBookingModal, slug }) {
  ...
  const handlePlacedOrder = () => {
    if (selectedDates.length === 0) {
      notificationWithIcon('error', 'ERROR', 'Minimum 1 date selection is required to placed an hostel booking order.');
    } else if (selectedDates.length > 5) {
      notificationWithIcon('error', 'ERROR', 'Maximum 5 days selection possible to placed an hostel booking order.');
    } else {
      confirm({
        title: 'Are your selected dates booked this Hostel?',
        icon: <ExclamationCircleOutlined />,
        okText: 'Ok',
        cancelText: 'Cancel',
        onOk() {
          return new Promise((resolve, reject) => {
            ApiService.post(`/api/v1/placed-hostelbooking-order/${bookingModal?.roomId}`, {
  ...
  return (
    <Modal
      title='Select data which dates are you Booked Hostel:'
...
HostelOrderPlaceModal.defaultProps = {
  bookingModal: { open: false, roomId: null }
};

HostelOrderPlaceModal.propTypes = {
  bookingModal: PropTypes.object
};

export default HostelOrderPlaceModal;
