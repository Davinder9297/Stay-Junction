...
function HostelBookingHistory() {
  ...
  const [loading, error, response] = useFetchData(`/api/v1/get-user-hostelbooking-orders?limit=${filter.limit}&page=${filter.page}&sort=${filter.sort}`, fetchAgain);
  ...
  const handleCancelBookingOrder = (id) => {
    confirm({
      ...
      content: 'Are you sure cancel your hostel booking order?',
      onOk() {
        return new Promise((resolve, reject) => {
          ApiService.put(`/api/v1/cancel-hostelbooking-order/${id}`)
            .then((res) => {
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              dataIndex: 'room',
              render: (data) => (
                <Link href={`/hostels/${data?.hostel_slug}`} target='_blank'>
                  <Button
                    className='btn-primary'
                    size='large'
                    type='link'
                  >
                    {data?.hostel_name || 'N/A'}
                  </Button>
                </Link>
              ),
              align: 'center'
...
export default HostelBookingHistory;
